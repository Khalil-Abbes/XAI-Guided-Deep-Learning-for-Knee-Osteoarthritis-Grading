"""
03_gradcam_analysis.py
-----------------------
Runs Grad-CAM on the full validation set and computes TWO JSER variants:

  JSER_SQUARE  : original square centre crop (rows 56-168, cols 56-168)
  JSER_HBAND   : anatomically motivated horizontal band (rows 84-140, full width)

Saves separate unfaithful-case JSONs and a side-by-side figure for each.
"""
...
model.load_state_dict(torch.load("data/baseline_resnet50.pth", map_location=device))
...
with open("data/xai_analysis/val_gradcam_results.json", "w") as f:
    json.dump(results, f, indent=2)
print("Saved: data/xai_analysis/val_gradcam_results.json")

# ── 8. Report + save unfaithful cases for EACH region ─────────────────────────
...
for key, region in REGIONS.items():
    ...
    print()
    print("=" * 55)
    print(f"FAITHFULNESS REPORT — {region['label'].upper()}")
    ...
    # Save unfaithful JSON
    out_path = f"data/xai_analysis/unfaithful_cases_{key}.json"
    with open(out_path, "w") as f:
        json.dump(unfaithful, f, indent=2)
    print(f"\nSaved: {out_path}  ({len(unfaithful)} cases)")

# ── 9. Region visualisation diagram ───────────────────────────────────────────
# Show both crop boxes overlaid on a sample knee X-ray
sample_path = results[0]["path"]
sample_img  = cv2.cvtColor(cv2.resize(cv2.imread(sample_path), (224, 224)),
                            cv2.COLOR_BGR2RGB)

fig, axes = plt.subplots(1, 2, figsize=(10, 5))
fig.suptitle("JSER Region Comparison", fontsize=14, fontweight="bold")

for ax, (key, region) in zip(axes, REGIONS.items()):
    ax.imshow(sample_img, cmap="gray")
    rect = patches.Rectangle(
        (region["col_min"], region["row_min"]),
        region["col_max"] - region["col_min"],
        region["row_max"] - region["row_min"],
        linewidth=2.5, edgecolor=region["color"], facecolor="none",
        linestyle="--"
    )
    ax.add_patch(rect)
    ax.set_title(f"{region['label']}\n{region['description']}", fontsize=9)
    ax.axis("off")

plt.tight_layout()
plt.savefig("data/xai_analysis/jser_region_comparison.png", dpi=150, bbox_inches="tight")
plt.close()
print("\nSaved: data/xai_analysis/jser_region_comparison.png")

# ── 10. Side-by-side worst-5 unfaithful figures for EACH region ───────────────
...
for key, region in REGIONS.items():
    ...
    fig.suptitle(
        f"5 Worst Unfaithful Cases — {region['label']}\n"
    ...
    for i, case in enumerate(top5):
        ...
        # Col 0: original with region box
        ...
        # Col 2: both JSER scores compared
        other_key  = "jser_hband" if key == "square" else "jser_square"
        bar_labels = ["Square\nCrop", "H-Band"]
        bar_vals   = [case["jser_square"], case["jser_hband"]]
        bar_colors = ["tomato", "limegreen"]
        axes[i, 2].bar(bar_labels, bar_vals, color=bar_colors, width=0.4)
    ...
    plt.tight_layout()
    out_fig = f"data/xai_analysis/unfaithful_examples_{key}.png"
